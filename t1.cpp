...
{ // Main logic for handling command-line arguments and running library operations. Includes file reading and writing, and executing library commands.
    ...
    std::string outputFilename = inputFilename + "_output_file.txt";
    ...
    while (std::getline(inputFileStream, line))
    {
        ...
        if (command == "InsertBook")
        {
            ...
            iss >> bookID >> title >> author >> availabilityStr;
            availabilityStr.erase(availabilityStr.end() - 1); // Remove the extra ')' character

            library.InsertBook(bookID, title, author, availabilityStr == "Yes", -1);
        }
        else if (command == "PrintBook")
        {
            std::string bookID;
            iss >> bookID;
            bookID.erase(bookID.end() - 1); // Remove the extra ')' character
            int bookIDInt = std::stoi(bookID);
            library.PrintBook(bookIDInt, outputFile);
        }
        else if (command == "PrintBooks")
        {
            std::string bookID1, bookID2;
            iss >> bookID1 >> bookID2;
            bookID2.erase(bookID2.end() - 1);
            int bookID1Int = std::stoi(bookID1);
            int bookID2Int = std::stoi(bookID2);

            library.PrintBooks(bookID1Int, bookID2Int, outputFile);
        }
        else if (command == "BorrowBook")
        {
            std::string patronID, bookID, patronPriority;
            iss >> patronID >> bookID >> patronPriority;
            patronPriority.erase(patronPriority.end() - 1);
            int patronIDInt = std::stoi(patronID);
            int bookIDInt = std::stoi(bookID);
            int patronPriorityInt = std::stoi(patronPriority);
            library.BorrowBook(patronIDInt, bookIDInt, patronPriorityInt, outputFile);
        }
        else if (command == "ReturnBook")
        {
            std::string patronID, bookID;
            iss >> patronID >> bookID;
            bookID.erase(bookID.end() - 1);
            int patronIDInt = std::stoi(patronID);
            int bookIDInt = std::stoi(bookID);
            library.ReturnBook(patronIDInt, bookIDInt, outputFile);
        }
        else if (command == "FindClosestBook")
        {
            std::string patronID;
            iss >> patronID;
            patronID.erase(patronID.end() - 1);
            int patronIDInt = std::stoi(patronID);
            library.FindClosestBook(patronIDInt, outputFile);
        }
        else if (command == "DeleteBook")
        {
            ...
            bookID.erase(bookID.end() - 1);
            int bookIDInt = std::stoi(bookID);
            library.DeleteBook(bookIDInt, outputFile);
        }
